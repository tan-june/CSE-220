...
Test(base_return, checksum02, .description="Compute a checksum") {
unsigned char packet[] =        "\x38\x91\x8a\x27\x31\x1e\x82\xca\xb0\x29\x20\x40\x00\x00\x00\x03\x1f\x00\x00\x24\x14\x54\xdb\x54\x54\x68\x65\x72\x65\x20\x61\x72\x65\x20\x74\x77";
    unsigned int actual = checksum_sf(packet);
    unsigned int expected = 0x1454db54;
    report_return_value(actual, expected);
}

Test(base_return, checksum03, .description="Compute a checksum") {
 unsigned char packet[] =         "\x38\x91\x8a\x27\x31\x1e\x82\xca\xb0\x29\x20\x40\x00\x00\x18\x03\x1f\x00\x00\x24\x14\x54\xdb\x6c\x69\x74\x65\x20\x65\x72\x72\x6f\x72\x2d\x66\x72";
   unsigned int actual = checksum_sf(packet);
    unsigned int expected = 0x1454db6c;
    report_return_value(actual, expected);
}

Test(base_return, checksum04, .description="Compute a checksum") {
unsigned char packet[] = "\x38\x91\x8a\x27\x31\x1e\x82\xca\xb0\x29\x20\x40\x00\x00\x0c\x03\x1f\x00\x00\x24\x14\x54\xdb\x60\x6f\x20\x77\x61\x79\x73\x20\x74\x6f\x20\x77\x72";
    unsigned int actual = checksum_sf(packet);
    unsigned int expected = 0x1454db60;
    report_return_value(actual, expected);
}

Test(base_return, checksum05, .description="Compute a checksum") {
 unsigned char packet[] =         "\x38\x91\x8a\x27\x31\x1e\x82\xca\xb0\x29\x20\x40\x00\x00\x30\x03\x1f\x00\x00\x24\x14\x54\xdb\x84\x20\x6f\x6e\x6c\x79\x20\x74\x68\x65\x20\x74\x68";
   unsigned int actual = checksum_sf(packet);
    unsigned int expected = 0x1454db84;
    report_return_value(actual, expected);
}

Test(base_return, checksum06, .description="Compute a checksum") {
...
Test(base_return, checksum07, .description="Compute a checksum") {
 unsigned char packet[] =         "\x38\x91\x8a\x27\x31\x1e\x82\xca\xb0\x29\x20\x40\x00\x00\x3c\x03\x1f\x00\x00\x25\x14\x54\xdb\x91\x69\x72\x64\x20\x6f\x6e\x65\x20\x77\x6f\x72\x6b";
   unsigned int actual = checksum_sf(packet);
    unsigned int expected = 0x1454db91;
    report_return_value(actual, expected);
}

Test(base_output, print_packet02, .description="Print the contents of a packet") {
    char *test_name = "print_packet02";
    run_using_system_no_valgrind(test_name); 
    expect_outfile_matches(test_name);
}

Test(base_output, print_packet03, .description="Print the contents of a packet") {
    char *test_name = "print_packet03";
    run_using_system_no_valgrind(test_name); 
    expect_outfile_matches(test_name);
}

Test(base_output, print_packet04, .description="Print the contents of a packet") {
    char *test_name = "print_packet04";
    run_using_system_no_valgrind(test_name); 
    expect_outfile_matches(test_name);
}

Test(base_output, print_packet05, .description="Print the contents of a packet") {
    char *test_name = "print_packet05";
    run_using_system_no_valgrind(test_name); 
    expect_outfile_matches(test_name);
}

Test(base_output, print_packet06, .description="Print the contents of a packet") {
    char *test_name = "print_packet06";
    run_using_system_no_valgrind(test_name); 
    expect_outfile_matches(test_name);
}

Test(base_return, packetize03, .description="Packetize a message that is too large to completely packetize.") {
    char msg[] = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAaAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA";
    unsigned char *packets_act[100];
    unsigned int packets_len = 5;
    unsigned int max_payload = 17;
    unsigned long src_addr = 5000000000L;
    unsigned long dest_addr = 16500000000L;
    unsigned short encryption = 265;
...
Test(base_return, packetize04, .description="Packetize a message that is too large to completely packetize.") {
    char msg[] = "hi";
    unsigned char *packets_act[10];
    unsigned int packets_len = 5;
    unsigned int max_payload = 1;
    unsigned long src_addr = 5000000000L;
    unsigned long dest_addr = 16500000000L;
    unsigned short encryption = 265;
    unsigned int num_packets_act = packetize_sf(msg, packets_act, packets_len, max_payload, src_addr, dest_addr, encryption);
...
Test(base_output, packetize05, .description="Packetize a message that is too large to completely packetize.") {
    char msg[] = "hi";
    unsigned char *packets_act[10];
    unsigned int packets_len = 5;
    unsigned int max_payload = 1;
    ...
    char *packets_exp[] = {
        "\x01\x2a\x05\xf2\x00\x03\xd7\x7a\x05\x00\x20\x40\x00\x00\x00\x01\x09\x00\x00\x19\x01\x7f\xf8\x87\x68",
        "\x01\x2a\x05\xf2\x00\x03\xd7\x7a\x05\x00\x20\x40\x00\x00\x01\x01\x09\x00\x00\x19\x01\x7f\xf8\x88\x69",
    };

    unsigned int packet_lens_exp[] = {25, 25};
    packetize_sf(msg, packets_act, packets_len, max_payload, src_addr, dest_addr, encryption);
    report_packets(packets_act, packets_exp, packet_lens_exp, num_packets_exp);
}
